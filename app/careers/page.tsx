...
// ── Values ──────────────────────────────────────────────────────────────────────
const VALUES = [
  { emoji: "🎯", title: "Purpose-Driven", desc: "We build coatings that protect and beautify — and people who take pride in it." },
  { emoji: "🤝", title: "People First", desc: "Every role matters. We invest in our people as much as our products." },
  { emoji: "🚀", title: "Always Improving", desc: "We embrace new tech and ideas to stay ahead in a changing industry." },
];
...
function QuarterCircle({ className, color, rotate = 0, style }: { className?: string; color: string; rotate?: number; style?: React.CSSProperties }) {
  return (
...
function FramedPhoto({
  src, size = 280, ringColor = "#16a34a", className = "",
}: { src: string; size?: number; ringColor?: string; className?: string }) {
  return (
...
export default function CareersPage() {
  ...
  return (
    <div className="min-h-screen bg-[#faf9f7] overflow-x-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-20">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <h1 className="text-[clamp(2.4rem,5.2vw,4rem)] font-bold text-stone-900 leading-[1.05]"
              ...
              <p className="mt-5 text-[16px] text-stone-500 max-w-md leading-relaxed">
                We bring talented people and a trusted coatings brand together to launch careers, drive innovation, and build community.
              </p>
              ...
              <div className="mt-10 grid grid-cols-4 gap-2 max-w-md">
                {STATS.map((s, i) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.1 }}>
                    <div className="text-2xl sm:text-[1.7rem] font-bold text-stone-900 tabular-nums" style={{ fontFamily: "var(--font-raleway), sans-serif", letterSpacing: "-0.03em" }}>
                      {s.value}
                    </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.15 }}
              ...
              <div className="relative grid grid-cols-2 grid-rows-2 gap-4 h-[440px]">
                ...
                <div className="row-span-2 rounded-[2rem] overflow-hidden shadow-lg border-4 border-white">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={IMG.heroMain} alt="Team collaboration at KMOPL" className="w-full h-full object-cover" />
                </div>
                {/* Top-right image */}
                <div className="rounded-[2rem] overflow-hidden shadow-lg border-4 border-white">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={IMG.heroA} alt="" className="w-full h-full object-cover" />
                </div>
                {/* Bottom-right image */}
                <div className="rounded-[2rem] overflow-hidden shadow-lg border-4 border-white">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={IMG.heroB} alt="" className="w-full h-full object-cover" />
                </div>
              </div>
      ...
      <section className="relative py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
            ...
            <span className="text-7xl font-bold text-stone-100 leading-none" style={{ fontFamily: "var(--font-raleway), sans-serif" }}>01</span>
            <span className="block text-xs font-bold uppercase tracking-[0.25em] text-amber-600 -mt-6 mb-3">For You</span>
            <h2 className="text-[clamp(1.8rem,3.6vw,2.6rem)] font-bold text-stone-900 leading-tight"
              style={{ fontFamily: "var(--font-raleway), sans-serif", letterSpacing: "-0.03em" }}>
              Access to a <span className="text-amber-500">Life-Long Career</span>
            </h2>
            <p className="mt-4 text-[15px] text-stone-500 leading-relaxed max-w-md">
              Turn your skills and ambition into a lasting career. We pair hands-on experience with mentorship and corporate connections so you keep growing — year after year.
            </p>
            <button onClick={scrollToOpenings}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mb-12">
            ...
            <h2 className="text-[clamp(1.8rem,3.6vw,2.8rem)] font-bold text-stone-900 leading-tight"
              style={{ fontFamily: "var(--font-raleway), sans-serif", letterSpacing: "-0.03em" }}>
              A workplace that helps you <span className="text-amber-500">thrive</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {PERKS.map((perk, i) => (
              <motion.div key={i}
                ...
                className="group p-7 rounded-3xl bg-white border border-stone-200/80 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <h3 className="text-[16px] font-bold text-stone-900 mb-2 leading-snug" style={{ fontFamily: "var(--font-raleway), sans-serif" }}>
                  {perk.title}
                </h3>
                <p className="text-[13.5px] text-stone-500 leading-relaxed">{perk.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-2 w-64 h-64 rounded-full" style={{ background: "#f59e0b", opacity: 0.12 }} />
            <QuarterCircle className="absolute left-6 top-0 w-16 h-16" color="#16a34a" />
          ...
          {/* Quote */}
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <span className="text-7xl font-bold text-stone-100 leading-none" style={{ fontFamily: "var(--font-raleway), sans-serif" }}>02</span>
            <span className="block text-xs font-bold uppercase tracking-[0.25em] text-amber-600 -mt-6 mb-3">Testimonial</span>
            <svg className="w-10 h-10 text-amber-300 mb-3" fill="currentColor" viewBox="0 0 24 24">
              <path d="M9.983 3v7.391c0 5.704-3.731 9.57-8.983 10.609l-.995-2.151c2.432-.917 3.995-3.638 3.995-5.849h-4v-10h9.983zm14.017 0v7.391c0 5.704-3.748 9.571-9 10.609l-.996-2.151c2.433-.917 3.996-3.638 3.996-5.849h-3.983v-10h9.983z"/>
            </svg>
            <p className="text-xl sm:text-2xl font-semibold text-stone-800 leading-snug" style={{ fontFamily: "var(--font-raleway), sans-serif" }}>
      ...
      {/* ═══════════════ VALUES / CULTURE ═══════════════ */}
      <section className="relative py-20 overflow-hidden" style={{ background: "linear-gradient(135deg, #1c1917 0%, #292524 60%, #1c1917 100%)" }}>
        <div aria-hidden className="absolute inset-0 pointer-events-none">
          <div className="absolute -top-32 -right-20 w-[420px] h-[420px] rounded-full opacity-10" style={{ background: "radial-gradient(circle, #f59e0b 0%, transparent 70%)" }} />
        </div>
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-14">
            ...
            <h2 className="text-[clamp(1.8rem,3.6vw,2.8rem)] font-bold text-white leading-tight"
              style={{ fontFamily: "var(--font-raleway), sans-serif", letterSpacing: "-0.03em" }}>
              What we stand for
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {VALUES.map((v, i) => (
              <motion.div key={i}
                ...
                <h3 className="text-lg font-bold text-white mb-2" style={{ fontFamily: "var(--font-raleway), sans-serif" }}>{v.title}</h3>
                <p className="text-[13.5px] text-white/45 leading-relaxed">{v.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* ═══════════════ CURRENT OPENINGS ═══════════════ */}
      <section ref={openingsRef} className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-20 scroll-mt-24">
        <div className="text-center mb-12">
          ...
          <p className="mt-3 text-[14px] text-stone-500">
            {loadingJobs ? "Loading roles…" : `${jobs.length} active position${jobs.length !== 1 ? "s" : ""} — find your fit.`}
          </p>
        </div>
        ...
        {loadingJobs ? (
          ...
          <div className="text-center py-16 bg-white rounded-3xl border border-stone-200/80">
            ...
            <p className="text-sm text-stone-400 mt-1">Send us your resume below — we'll reach out when a role opens up.</p>
            <button onClick={() => scrollToForm()} className="mt-5 inline-flex items-center gap-2 px-6 py-3 rounded-full text-white font-bold text-sm"
              style={{ background: "linear-gradient(135deg, #f59e0b, #d97706)" }}>
                ...
                <div className="p-6 sm:p-7">
                  ...
                  {job.qualification && (
                    <p className="mt-4 text-[13.5px] text-stone-600">
                      <span className="font-semibold text-stone-800">Qualification:</span> {job.qualification}
                    </p>
                  )}
                  {job.description && (
                    <p className="mt-2 text-[13.5px] text-stone-500 leading-relaxed">{job.description}</p>
                  )}

                  <div className="mt-5 flex items-center justify-between flex-wrap gap-3 pt-4 border-t border-stone-100">
                    {job.createdAt ? (
                      <span className="text-[12px] text-stone-400 font-medium">Posted on {formatPostedDate(job.createdAt)}</span>
                    ) : <span />}
                    <div className="flex items-center gap-2">
                      ...
                      <a href={`mailto:${job.applyEmail || "hr@kmopl.com"}?subject=${encodeURIComponent("Application for " + job.title)}`}
      ...
      {/* ═══════════════ APPLICATION FORM ═══════════════ */}
      <section ref={formSectionRef} className="relative py-20 overflow-hidden scroll-mt-24"
        style={{ background: "linear-gradient(160deg, #fefdfb 0%, #f0e6d4 60%, #faf8f4 100%)" }}>
        <div aria-hidden className="absolute inset-0 pointer-events-none">
          <div className="absolute -top-24 -right-24 w-[440px] h-[440px] rounded-full opacity-20" style={{ background: "radial-gradient(circle, #fde68a 0%, transparent 70%)" }} />
          <QuarterCircle className="absolute left-6 bottom-10 w-20 h-20 opacity-40" color="#16a34a" rotate={90} />
        </div>
