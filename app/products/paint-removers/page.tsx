...
const ORANGE = "#ea580c";
const HERO_BADGES = [
  { icon: "🎨", label: "Effective Removal" },
  { icon: "☀️", label: "Surface Safe" },
  { icon: "⚗️", label: "Multi-Substrate" },
  { icon: "🏭", label: "Industrial Grade" },
  { icon: "✅", label: "Consistent Quality" },
];
export default function DyestuffPage() {
  const { products } = useProducts();
  const items = products.filter((p) => p.substrate === "paint-removers");
      ...
      <section className="relative overflow-hidden min-h-[400px]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="max-w-lg">
            <h1 className="text-[clamp(2rem,4.5vw,3.2rem)] font-extrabold leading-[1.1] mb-4" style={{ fontFamily: "var(--font-raleway), sans-serif", color: NAVY }}>Paint Removers<br /><span style={{ color: ORANGE }}>For Every Application</span></h1>
            <p className="text-stone-600 text-[15px] leading-relaxed mb-6 max-w-md">Effective paint removers formulated as per substrate requirements for wood, metal, and glass surfaces.</p>
      ...
      <section ref={ref} className="bg-[#f8f9fa] py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-extrabold mb-2" style={{ fontFamily: "var(--font-raleway), sans-serif", color: NAVY }}>Our Paint Remover Products</h2>
            <p className="text-stone-500 text-[14px]">Substrate-specific paint removal solutions.</p>
          </motion.div>
          {items.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {items.map((p, i) => (<Card key={p.id} product={p} index={i} inView={inView} substrate="paint-removers" />))}
            </div>
          )}
        </div>
      </section>
