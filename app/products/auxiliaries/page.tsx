...
const HERO_BADGES = [
  ...
  { icon: "☀️", label: "Easy Application" },
  { icon: "⚗️", label: "Enhanced Finish" },
  { icon: "🏭", label: "Industrial Grade" },
  { icon: "✅", label: "Consistent Quality" },
];
export default function DyestuffPage() {
  const { products } = useProducts();
  const items = products.filter((p) => p.substrate === "auxiliaries");
  const ref = useRef<HTMLElement>(null);
  const inView = useInView(ref, { once: true, margin: "-60px" });
  return (
    <div className="min-h-screen bg-white">
      <section className="relative overflow-hidden min-h-[400px]">
        <div className="absolute inset-0"><img src="/paint-images/images/5.jpg" alt="Dyestuff" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-gradient-to-r from-white via-white/90 to-transparent" /></div>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="max-w-lg">
            <h1 className="text-[clamp(2rem,4.5vw,3.2rem)] font-extrabold leading-[1.1] mb-4" style={{ fontFamily: "var(--font-raleway), sans-serif", color: NAVY }}>Wood Auxiliaries<br /><span style={{ color: ORANGE }}>For Every Application</span></h1>
            <p className="text-stone-600 text-[15px] leading-relaxed mb-6 max-w-md">Complementary products that enhance coating performance, application ease, and final finish quality for wood substrates.</p>
      ...
      <section ref={ref} className="bg-[#f8f9fa] py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-extrabold mb-2" style={{ fontFamily: "var(--font-raleway), sans-serif", color: NAVY }}>Our Wood Auxiliaries Products</h2>
            <p className="text-stone-500 text-[14px]">Complementary products for enhanced wood coating performance.</p>
          </motion.div>
          {items.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {items.map((p, i) => (<Card key={p.id} product={p} index={i} inView={inView} substrate="auxiliaries" />))}
            </div>
          )}
        </div>
      </section>
